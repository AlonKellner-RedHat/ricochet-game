...
import { tracePhysicalPath, unifiedToAlignment, unifiedToPathResult } from "@/trajectory-v2/engine/PathBuilder";
...
describe("UnifiedPath Architecture", () => {
  ...
  describe("deriveRender", () => {
    ...
    it("should produce red segments when diverged", () => {
      ...
      const planned = createTestSurface(
        "planned",
        { x: 250, y: 100 },
        { x: 250, y: 300 },
        true
...
describe("First Principle: Solid path from player to cursor", () => {
  it("should always have solid segments reaching cursor when path is aligned", () => {
    ...
    const distToCursor = Math.sqrt(
      Math.pow(lastSolid!.end.x - cursor.x, 2) + 
      Math.pow(lastSolid!.end.y - cursor.y, 2)
    );
...
describe("First Principle: Planned path reflects only off planned surfaces", () => {
  it("should mark segment as diverged when hitting non-planned surface", () => {
    const player = { x: 100, y: 200 };
    const cursor = { x: 300, y: 200 };
    ...
    // The path should diverge AFTER hitting the non-planned surface
    expect(path.firstDivergedIndex).not.toBe(-1);
    
    // FIRST PRINCIPLE: Segment ENDING at the surface should be "unplanned" (green)
    // because both planned and actual paths go in the same direction until they hit it
    const hitSegment = path.segments.find(s => s.endSurface?.id === "ricochet");
    expect(hitSegment).toBeDefined();
    expect(hitSegment!.planAlignment).toBe("unplanned"); // Green - aligned portion
    
    // The segment AFTER the reflection should be "diverged" (red)
    ...
    if (path.segments.length > hitIndex + 1) {
      ...
      expect(nextSegment!.planAlignment).toBe("diverged"); // Red - diverged portion
    }
  });

  it("should render green up to non-planned surface, then red to cursor", () => {
    const player = { x: 100, y: 200 };
    const cursor = { x: 300, y: 200 };
    ...
    // Last solid segment should be RED (diverged) and end at cursor
    const lastSolid = solidSegments[solidSegments.length - 1];
    expect(lastSolid).toBeDefined();
    expect(lastSolid!.color).toBe("red");
    expect(lastSolid!.end.x).toBe(cursor.x);
...
describe("First Principle: Segment ending at unplanned surface should be green", () => {
  it("should mark segment to unplanned reflective surface as unplanned (green)", () => {
    ...
    // If there's a second segment (after reflection), it should be diverged
    if (path.segments.length >= 2) {
      const secondSegment = path.segments[1]!;
      expect(secondSegment.planAlignment).toBe("diverged");
    }
  });
