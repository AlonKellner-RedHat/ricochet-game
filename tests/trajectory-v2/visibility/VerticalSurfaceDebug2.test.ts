/**
 * Debug test for vertical surface cone boundary issues - failing player position.
 */
...
import { isHitPoint } from "@/trajectory-v2/geometry/SourcePoint";

describe("Vertical Surface Debug 2", () => {
  const SCREEN_BOUNDS = { minX: 0, minY: 0, maxX: 1280, maxY: 720 };

  // Vertical surface - same as invariant test
  const VERTICAL_SURFACE = createRicochetChain("v1", [
    { x: 400, y: 260 },
    { x: 400, y: 460 },
  ]);

  it("should analyze reflected cone for player=(1091,666)", () => {
    const player = { x: 1091, y: 666 };
    ...
    sourcePoints.forEach((p, i) => {
      ...
      if (isHitPoint(p)) {
        type = `HitPoint(${(p as any).hitSurface?.id})`;
      }
